...
- DOM manipulation
  ...
  - Exercises: [Favorite dishes](#favorite-dishes), [podcasts](#podcasts), [image inserter](#image-inserter)

- Event listeners - Focus on usage, no explanation of callback
  - Click, mouseover, etc. Explain one and let students investigate another.
  - Input Change, input
  - Event parameter
...
Create a function that has two parameters: `imageUrl` and `elementToAppendImageTo`. The function should create a img tag using the imageUrl. The function should then append the `img` to the `elementToAppendImageTo` - html element.

```js
// Should append a ima to the body with the picture from 'https://picsum.photos/536/354'
